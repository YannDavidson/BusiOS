...
export interface PortalStore {
  ...
  agentLimit(businessId: string): Promise<number>;
}

export class MemoryPortalStore implements PortalStore {
  users = new Map<string, { id: string; email?: string }>(); businesses = new Map<string, PortalBusiness>(); memberships = new Map<string, PortalRole>(); voice = new Map<string, Record<string, unknown>>(); agents = new Map<string, Record<string, unknown>>(); limits = new Map<string, number>();
  ...
  async agentLimit(businessId: string) { return this.limits.get(businessId) ?? 8; }
}
...
export class SupabasePortalStore implements PortalStore {
  ...
  async agentLimit(businessId: string) { const { data, error } = await this.client.from('billing_accounts').select('plan_code,status,trial_ends_at').eq('business_id', businessId).maybeSingle(); if (error) throw error; if (data && data.status !== 'active') return 2; const plan = data?.plan_code ?? 'pilot'; if (plan === 'pilot' && data?.trial_ends_at && new Date(data.trial_ends_at).getTime() <= Date.now()) return 2; return ({ basic: 2, growth: 4, business: 8, custom: 100, pilot: 8 } as Record<string, number>)[plan] ?? 2; }
}
...
export class OwnerPortalService {
  ...
  async saveAgents(token: string, businessId: string, value: Record<string, unknown>) { await this.authorizeAdmin(token, businessId); const enabled = [...new Set(Array.isArray(value.enabled) ? value.enabled.map(String) : [])]; if (!enabled.includes('DIEGO') || enabled.some((id) => !agentIds.includes(id as typeof agentIds[number]))) throw new Error('Agent configuration must include Diego and only registered agents'); const limit = await this.store.agentLimit(businessId); if (enabled.length > limit) throw new Error(`Your subscription includes ${limit} agents`); await this.store.saveAgents(businessId, { enabled }); }
  async authorizeOwner(token: string, businessId: string) { const result = await this.authorize(token, businessId); if (result.role !== 'owner') throw new Error('Business owner access required'); return result.user; }
