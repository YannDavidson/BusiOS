...
import type { CallSession, CallStatus, CallSummary, RealtimeEventType, VoiceAction, VoiceNumber } from './types.js';
...
export interface VoiceStore {
  ...
  addRealtimeEvent(callSid: string, businessId: string, type: RealtimeEventType, payload: Record<string, unknown>): Promise<void>;
}

export class MemoryVoiceStore implements VoiceStore {
  numbers = new Map<string, VoiceNumber>(); sessions = new Map<string, CallSession>(); actions: VoiceAction[] = []; summaries = new Map<string, CallSummary>(); realtimeEvents: Array<{ callSid: string; businessId: string; type: RealtimeEventType; payload: Record<string, unknown> }> = [];
  ...
  async addRealtimeEvent(callSid: string, businessId: string, type: RealtimeEventType, payload: Record<string, unknown>) { this.realtimeEvents.push({ callSid, businessId, type, payload: structuredClone(payload) }); }
}

export class SupabaseVoiceStore implements VoiceStore {
  constructor(private client: SupabaseClient) {}
  async findNumber(phoneNumber: string) { const { data, error } = await this.client.from('voice_numbers').select('*, voice_agent_settings(*)').eq('phone_number', phoneNumber).eq('active', true).maybeSingle(); if (error) throw error; if (!data) return null; const settings = Array.isArray(data.voice_agent_settings) ? data.voice_agent_settings[0] : data.voice_agent_settings; return { businessId: data.business_id, phoneNumber: data.phone_number, businessName: settings?.business_name ?? 'the business', language: settings?.language ?? 'en', greeting: settings?.greeting ?? 'Thank you for calling. I am Marisol, the AI receptionist. How may I help?', transferNumber: settings?.transfer_number ?? undefined, fallbackMessage: settings?.fallback_message ?? 'I can take a message or connect you with a person.', faqs: settings?.faqs ?? [], active: data.active, realtimeEnabled: settings?.realtime_enabled ?? false, liveVoice: settings?.live_voice ?? 'Kore', maxDurationSeconds: settings?.max_duration_seconds ?? 600, maxTurns: settings?.max_turns ?? 30, maxAudioSeconds: settings?.max_audio_seconds ?? 900, maxLatencyMs: settings?.max_latency_ms ?? 750 }; }
  ...
  async addRealtimeEvent(callSid: string, businessId: string, type: RealtimeEventType, payload: Record<string, unknown>) { const { error } = await this.client.from('realtime_voice_events').insert({ call_sid: callSid, business_id: businessId, event_type: type, payload }); if (error) throw error; }
}
